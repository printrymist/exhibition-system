...
- バージョンフッタに **「✉ お問い合わせ」** リンクを併設。`inquiry.html` は既に
  実装済だがどこからもリンクされていなかった。フッタは全画面に出るので、
  どの画面からでも 1 クリックで問い合わせフォームに飛べる。
- 現在の展覧会コード (`?ex=`) を引き継いで `inquiry.html?ex=…` に渡すので、
  問い合わせ先のコンテキストが自動的に伝わる。
