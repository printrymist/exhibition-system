...
6. **端末に保存する値 (localStorage / cookie / IndexedDB) は「会場の共用タブレット等で前の人の値が焼き付く」前提で扱う** — 焼き付いて困るなら有効期限を付ける (`{v, ts}` 保存 + 読み込み時に失効判定、端末時計の未来ずれに注意)、意図的に永続なら理由をコメントに残す。詳細と実装例・触ってはいけない永続キー一覧は memory `feedback_localstorage_ttl` を参照。
